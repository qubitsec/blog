...
| No. | 질문 | 답변 |
| ---: | --- | --- |
| 1 | SSL VPN이 공개키 암호를 사용하는가? | 예. 일반적으로 인증서와 TLS 키 합의에 사용합니다. |
| 2 | 공개키 암호가 장기적으로 PQC 전환 대상인가? | 예. RSA·DH·ECDH·ECDSA 계열은 전환 대상입니다. |
| 3 | 그래서 모든 SSL VPN을 지금 교체해야 하는가? | 아닙니다. |
| 4 | 지금 새 PQC VPN 제품을 사야 하는가? | 사용 목적·데이터·표준 지원 여부를 보기 전에는 그런 결론을 낼 수 없습니다. |
...
| No. | VPN 사용 사례 | PQC 관점 검토 필요성 | 현재 보안 우선순위 | 이유 |
| ---: | --- | --- | --- | --- |
| 1 | 국가기밀·군사·외교 시스템 원격 접속 | 별도 우선 검토 | 매우 높음 | 장기 비밀성과 전략 가치가 클 수 있음 |
| 2 | 핵심 연구개발망·설계자료 저장소 접속 | 선별 검토 | 매우 높음 | 일부 자료는 장기간 가치가 유지될 수 있음 |
| 3 | 중요 인프라 관리자 원격 접속 | 데이터에 따라 검토 | 매우 높음 | 현재 계정·권한·장비 침해 위험이 큼 |
| 4 | 장기간 운영되는 사이트 간 VPN | 전송 데이터에 따라 검토 | 높음 | 지속적으로 고가치 데이터가 흐를 수 있음 |
| 5 | 일반 임직원 사내 업무 접속 | 낮음~선별 검토 | 높음 | HNDL보다 계정 탈취와 단말 침해가 현실적임 |
| 6 | SaaS·웹메일·그룹웨어 접속 | 대체로 낮음 | 높음 | 단기 세션과 현재 인증 보안이 더 중요함 |
| 7 | 만료 토큰·일회성 요청 중심 통신 | 낮음 | 중간~높음 | 수십 년 뒤 정보 가치가 낮음 |
...
| No. | 분류 축 | 확인 질문 |
| ---: | --- | --- |
| 1 | 사용자 | 일반 직원인가, 관리자·협력사·특권 계정인가? |
| 2 | 단말 | 회사 관리 단말인가, 개인·외부 단말인가? |
| 3 | 접속 대상 | 일반 업무 앱인가, 핵심 서버·관리망인가? |
| 4 | 권한 | 조회 권한인가, 변경·배포·관리 권한인가? |
| 5 | 데이터 | 단기 업무 데이터인가, 장기 고가치 자료인가? |
| 6 | 세션 | 일시적 원격접속인가, 상시 터널인가? |
| 7 | 노출 | 인터넷에서 직접 접근 가능한 게이트웨이인가? |
...
| No. | 구분 | 일반적 특징 | PQC 검토 포인트 |
| ---: | --- | --- | --- |
| 1 | 원격접속 VPN | 사용자별·세션별 접속 | 사용자 권한, 접속 대상, 세션 데이터 가치 |
| 2 | 사이트 간 VPN | 장기·상시 연결 가능 | 지속적으로 흐르는 업무·백업·관리 데이터의 가치 |
| 3 | 관리자 전용 VPN | 특권 계정 중심 | 현재 계정 탈취·권한 오남용이 최우선 |
| 4 | 협력사 VPN | 외부 조직과 연결 | 신뢰 경계, 계정 수명, 접근 범위가 중요 |
| 5 | 클라우드 연결 VPN | 하이브리드 인프라 연결 | 장기 운영, 벤더 지원, 대역폭·호환성 확인 |
...
| No. | 기능 | PQC와의 관계 |
| ---: | --- | --- |
| 1 | MFA | 현재 계정 탈취 대응 기능. PQC와 별개입니다. |
| 2 | 단말 신뢰성 검증 | 감염·미관리 단말 통제 기능. PQC와 별개입니다. |
| 3 | 최소 권한 | 접근 범위 제한 원칙. PQC와 별개입니다. |
| 4 | 세션 모니터링 | 접속 이후 이상 행위 확인. PQC와 별개입니다. |
| 5 | 하이브리드 키 합의 | PQC 전환과 직접 관련됩니다. |
| 6 | PQ 인증서·전자서명 | PQC 전환과 직접 관련됩니다. |
...
| No. | 영역 | 기존 방식 예시 | 역할 | PQC 영향 |
| ---: | --- | --- | --- | --- |
| 1 | 인증서·서명 | RSA, ECDSA | 서버·클라이언트 신원 인증 | 장기적으로 PQ 서명 전환 대상 |
| 2 | 키 합의 | DH, ECDH, X25519 | 데이터 채널 키를 만들 공유 비밀 합의 | ML-KEM 하이브리드 전환 대상 |
| 3 | 실제 데이터 암호화 | AES-GCM, ChaCha20-Poly1305 | 터널 패킷 암호화 | 직접적인 교체 대상 아님 |
...
| No. | 확인 항목 | 이유 |
| ---: | --- | --- |
| 1 | 서버 OpenVPN 버전 | 2.7 계열 기능 지원 여부 확인 |
| 2 | 클라이언트 OpenVPN 버전 | 구형 클라이언트와 상호운용성 확인 |
| 3 | 연결된 TLS 라이브러리 | OpenSSL 3.5 등 실제 ML-KEM 지원 여부 확인 |
| 4 | 빌드 옵션·배포판 | 기능이 포함돼 빌드되었는지 확인 |
| 5 | 허용 TLS 그룹 | `X25519MLKEM768` 협상 가능 여부 확인 |
| 6 | 실제 협상 결과 | 지원 목록과 실제 사용은 다를 수 있음 |
| 7 | 인증서 알고리즘 | RSA·ECDSA 인증서가 그대로 남아 있을 수 있음 |
| 8 | 자체 CA·CRL·발급 체계 | PKI 전환은 별도 과제임 |
| 9 | 모바일·임베디드 클라이언트 | 지원 시차와 배포 난이도 확인 |
| 10 | MTU·성능·연결 안정성 | 키 공유 크기 증가에 따른 영향 확인 |
