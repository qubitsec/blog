...
draft: false
title: "루트킷 탐지를 위한 방안"
description: "KISA가 공개한 리눅스 커널 루트킷 점검 스크립트의 의미와 한계를 살펴보고, 파일 기반 탐지만으로는 확인하기 어려운 커널 메모리 영역의 모듈 로드 행위를 어떻게 보완 탐지할 수 있는지 정리합니다."
featured_image: "cdn/tech/rootkit-detection-strategy.png"
tags: ["Linux 보안", "Rootkit", "Kernel Module", "modprobe", "KISA", "EDR", "XDR", "포렌식"]
---

## 루트킷 탐지, 파일만 보면 충분할까요?

KISA는 최근 리눅스 커널 루트킷 점검을 위한 가이드와 스크립트를 공개했습니다.

이는 매우 의미 있는 조치입니다. 루트킷은 일반적인 사용자 명령어만으로는 확인하기 어렵고, 침해 이후에도 자신을 숨기도록 설계되는 경우가 많기 때문입니다.
...
KISA가 공개한 루트킷 점검 스크립트는 주로 다음 영역을 확인합니다.

- 부팅 및 서비스 디렉터리
- 파일시스템 inode 정보
- 사용자 레벨에서 보이는 파일 목록
- 이전 점검 결과와 현재 결과의 차이
- 서비스에 등록된 의심 파일 또는 실행 흔적

즉, 핵심은 **파일 기반 탐지**입니다.

파일시스템에 존재하는 숨김 파일, 서비스 등록 흔적, 자동 실행 등록 여부를 비교하여 루트킷 감염 가능성을 확인하는 방식입니다.

이 방식은 매우 유용합니다. 특히 루트킷이 디스크에 파일을 남기거나, 부팅 시 자동 실행되도록 서비스를 등록하는 경우에는 효과적으로 이상 징후를 찾을 수 있습니다.
...
`modprobe`는 커널 모듈을 로드하는 정상 리눅스 명령입니다.
...
- 필요한 의존성 확인
- 커널 메모리에 모듈 적재
- `/proc/modules`, `/sys/module/`, `lsmod` 결과에 반영

중요한 점은 이것입니다.

> `modprobe` 실행은 파일을 새로 생성하는 행위가 아니라, 이미 존재하는 커널 모듈을 메모리에 올리는 행위입니다.

따라서 파일시스템 중심으로 동작하는 스캐너에서는 이 행위가 보이지 않을 수 있습니다.

| 구분 | 주로 보는 대상 | 대표 확인 항목 |
|---:|---|---|
| 1 | 파일 기반 Rootkit Scanner | inode, 숨김 파일, 서비스 등록, 자동 실행 파일 |
| 2 | `modprobe` 기반 모듈 로드 | 커널 메모리, 로드된 모듈 목록, 런타임 상태 |
| 3 | 커널 루트킷 탐지 | 커널 모듈 상태, 시스템 콜 변조, 커널 이벤트, 메모리 포렌식 |

즉, 파일 기반 점검 도구는 **디스크에 남은 흔적**을 찾는 데 강하고, `modprobe`와 같은 커널 모듈 로드는 **런타임 상태**를 확인해야 보입니다.
...
- 하지만 새로운 서비스 파일이나 자동 실행 파일이 생성된 것은 아님
...
따라서 루트킷 탐지도 **보이는 파일**만이 아니라, **보이지 않는 커널 상태**까지 확인하는 방향으로 확장되어야 합니다.

---

## 📖 참고 자료

- [KISA 보호나라&KrCERT/CC - 리눅스 커널 루트킷 점검 가이드 배포](https://www.krcert.or.kr/kr/bbs/view.do?bbsId=B0000133&nttId=71917&menuNo=205020)
