...
WAF는 단순히 웹 요청을 차단하는 장비형 웹방화벽 관점에 머무르지 않습니다.
...
이때 유용한 방식이 바로 **WAF 뒤에 WebProxy를 두고, 최종 Web 서비스가 반드시 그 경로를 거치도록 구성하는 방식**입니다.
...
핵심은 단순합니다.  
외부에서 들어오는 모든 웹 트래픽이 **WAF를 통과한 뒤**, 내부에서 다시 **WebProxy를 경유하여 Web으로 전달되도록 강제**하는 것입니다.
...
방화벽이 네트워크 레벨의 1차 통제를 담당하고, WAF가 HTTP/HTTPS 트래픽에 대한 탐지·차단을 수행한 뒤, 최종적으로 Web 서버가 요청을 처리합니다.
...
WAF 뒤에 바로 Web 서버가 위치하면, 애플리케이션 라우팅, 서비스 분리, 버전 전환, 트래픽 분산, 내부 서비스 연결 같은 운영 요구를 유연하게 수용하기 어려울 수 있습니다.
...
이제 WAF 뒤에 WebProxy를 두는 구조를 보겠습니다.
...
* 웹 공격에 대한 탐지·차단은 **WAF**가 담당합니다.
* 정상 처리 대상 트래픽은 **WebProxy**로 전달됩니다.
* WebProxy가 실제 **Web 서비스**로 전달합니다.

즉, WAF는 **보안 통제의 중심**이 되고, WebProxy는 **서비스 전달과 내부 라우팅의 중심**이 됩니다.
...
WAF 뒤에 WebProxy를 두면, 최종 Web 서비스는 외부와 직접 연결되지 않고 항상 Proxy를 통해서만 접근하게 할 수 있습니다.
...
이렇게 하면 서비스 운영자는 WebProxy 계층에서 애플리케이션 전달 정책을 관리하고, 보안 운영자는 WAF 계층에서 탐지·차단 정책을 관리할 수 있습니다.
...
즉, WAF가 “막는 역할”에 집중하고, WebProxy가 “보내는 역할”에 집중하게 됩니다.
...
이 경우 WAF가 외부 공격을 통제하고, 그 뒤에서 쿠버네티스 기반 WebProxy가 실제 서비스 Pod로 전달하는 구조가 됩니다.
...
WAF + WebProxy 구조는 특히 다음과 같은 환경에 적합합니다.
...
WAF를 WebProxy 구조로 사용할 때 핵심은 다음과 같습니다.

### 첫째, 진입점은 WAF로 고정합니다

외부 요청은 반드시 WAF를 거치도록 해야 합니다.  
...
WAF를 WebProxy 형태로 활용하는 구조는 단순히 중간 계층을 하나 더 두는 이야기가 아닙니다.
