...
tags: ["WAF", "웹방화벽", "SaaS", "보안 체크리스트", "XDR", "EDR"]
---
...
### 4. JWT/세션 기반 인증 우회 공격을 탐지할 수 있는가
...
### 6. 데이터 유출(Exfiltration) 탐지가 가능한가
...
### 8. 전수 로그 분석(Full Logging)이 가능한가
...
### 10. 통합 분석 플랫폼(XDR/EDR/SIEM 등)과 연계 가능한가
...
| 항목 | 필수 여부 | 확인 방법 | 핵심 포인트 |
|------|----------|----------|------------|
| 요청 본문 분석 | 필수 | POST/JSON/XML 시연 | 공격 payload 가시성 |
| 크리덴셜 스터핑 탐지 | 필수 | 로그인 분산 공격 시나리오 질문 | 계정 탈취 대응 |
| JWT/세션 공격 탐지 | 필수 | 토큰 재사용/우회 시나리오 확인 | 인증 우회 방지 |
| 제로데이 대응 | 필수 | 시그니처 없는 공격 대응 질문 | 행위 기반 분석 |
| 데이터 유출 탐지 | 필수 | 응답 본문·대량 조회 탐지 여부 | 내부 정보 보호 |
| 웹쉘/서버 연계 | 필수 | 웹루트 변경·자식 프로세스 감시 확인 | 침해 이후 대응 |
| 전수 로그 저장 | 필수 | 샘플링 여부·보관 기간 확인 | 사후 재분석 |
| 실시간 대응 | 필수 | 자동 차단/세션 종료 가능 여부 | 공격 차단 |
| 통합 분석 연계 | 필수 | API/Webhook/SIEM 연동 확인 | 전체 공격 흐름 분석 |
