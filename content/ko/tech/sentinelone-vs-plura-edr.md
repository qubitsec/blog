...
| 9 | 실시간 포렌식 연동 | Storyline과 Deep Visibility로 조사·대응 가속 | 탐지와 동시에 근거 로그와 행위 흐름을 바로 확인하는 구조 |
| 10 | 웹 공격 대응 논리 | 공식 설명의 중심은 엔드포인트 보호 | XDR 확장 시 WAF와 직접 연결 |
| 11 | 크리덴셜 스터핑 대응 | 제품 설명의 중심 축은 아님 | 웹 로그·필터 기반 탐지와 차단 구조 설명 |
| 12 | 요청/응답 본문 분석 | 공식 설명 중심 아님 | 웹 요청/응답 본문 로그 분석 강조 |
| 13 | 운영 관점 | 엔드포인트 보호와 조사 자동화 중심 | 웹과 호스트 행위를 하나의 공격 흐름으로 추적 |
| 14 | AI 해킹 시대 관점 | 행위 기반 보호 + 조사 가속 + 자동 대응 | 고속·다단계 공격에 대해 웹부터 호스트까지 연속 대응 논리 |
| 15 | 핵심 한계 또는 강점 | 강력한 EDR이지만 웹 공격까지 자연스럽게 연결되는 XDR 논리는 약함 | 엔드포인트 단독을 넘어 웹 공격 차단까지 확장 가능 |
