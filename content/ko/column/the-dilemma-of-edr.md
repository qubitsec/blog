...
특히 커널 모드 사용 여부는 벤더가 공개 문서에서 명확히 밝히지 않는 경우가 많습니다. 따라서 아래 평가는 공개 자료와 제품 기능 구조를 기준으로 한 판단이며, 실제 도입 시에는 벤더에 커널 드라이버 사용 여부, 드라이버명, 로드 조건, 업데이트 방식, 장애 시 복구 절차를 별도로 확인해야 합니다.

| 제품 | 제품 성격 | 공개자료 기준 커널 모드 판단 | 판단 근거 | 블로그용 표현 |
|---|---|---|---|---|
| **PLURA-EDR / PLURA-XDR** | 상세 로그·행위 기반 EDR, XDR 확장 구조 | **커널 모드 중심 제품으로 보기 어려움** | 운영체제 이벤트, 감사 로그, 시스템 로그, 웹 요청·응답 로그, 포렌식 증거와의 상관분석을 중심으로 설명됨 | 커널 깊숙이 들어가는 감시자보다, OS와 웹에서 발생한 증거를 수집·상관분석하는 구조 |
