...
| 구분     | 주요 행위                          |
| ------ | ------------------------------ |
| 실행 행위  | 악성코드 실행, 스크립트 실행, 프로세스 생성      |
| 시스템 변경 | 파일 생성·수정·삭제, 레지스트리 조작          |
| 공격 확장  | 권한 상승, 메모리 인젝션, DLL 인젝션        |
| 외부 통신  | C2 연결, 데이터 유출, 비정상 네트워크 접속     |
| 침해 흔적  | 웹셸 실행, 서버 명령 실행, 계정 탈취 후 내부 이동 |

이러한 행위는 서버와 PC에서 직접 발생합니다.

따라서 엔드포인트에서 일어나는 행위를 보지 않고는 실제 침해 여부를 판단하기 어렵습니다.

그래서 EDR은 필요합니다.

문제는 **EDR이 공격을 보기 위해 얼마나 깊은 권한으로 동작하는가**입니다.

EDR과 안티바이러스 제품은 더 깊은 탐지를 위해 운영체제 커널 모드에서 동작하거나, 다른 프로세스의 가상 메모리에 접근하거나, 파일·프로세스·네트워크 흐름을 가로채는 구조를 사용하기도 합니다.

이것은 강력한 탐지 기능이 될 수 있습니다.  
그러나 동시에 매우 민감한 위험 요소가 될 수 있습니다.
...
flowchart TB
    ...
    U1 --> R1[사용자 모드 오류: 해당 앱 중심 장애]
    K4 --> R2[커널 모드 오류: OS 전체 장애 가능]
```

그래서 커널 모드 보안 제품은 단순히 “탐지를 잘하는가”만 볼 것이 아니라, “커널에 꼭 있어야 하는가”, “오류가 발생했을 때 어디까지 영향을 주는가”까지 함께 보아야 합니다.
...
조금 더 구체적으로 보면, 문제는 Windows 센서에 배포된 `Channel File 291` 관련 Rapid Response Content 업데이트에서 발생했습니다. CrowdStrike 설명에 따르면 이 업데이트는 새로운 공격 기법을 관찰하기 위한 IPC(InterProcess Communication) 관련 Template Instance를 포함하고 있었습니다.

CrowdStrike의 설명을 기준으로 사고 흐름을 정리하면 다음과 같습니다.

1. Falcon Sensor 7.11에서 IPC Template Type이 도입되었습니다.
2. Template Type은 위협 탐지 엔지니어가 Rapid Response Content에서 활용할 수 있도록 미리 정의된 필드 구조를 갖고 있었습니다.
3. Rapid Response Content는 센서 코드 자체가 아니라, 센서가 해석하는 설정 데이터 형태의 Template Instance였습니다.
4. 2024년 7월 19일 배포된 Template Instance 중 하나가 Content Validator의 버그로 인해 문제가 있는 데이터임에도 검증을 통과했습니다.
5. 센서는 20개의 입력 필드를 기대했지만, 해당 업데이트는 21개의 입력 필드를 제공했습니다.
6. 이 불일치가 Content Interpreter에서 out-of-bounds memory read를 유발했고, 예외가 정상적으로 처리되지 못하면서 Windows BSOD로 이어졌습니다.

```mermaid
flowchart TD
    A[Rapid Response Content 생성] --> B[Template Instance 구성]
    B --> C[Content Validator 검증]
    C -->|문제 있는 데이터 통과| D[Channel File 291 배포]
    D --> E[Windows Sensor 수신]
    E --> F[Content Interpreter 로드]
    F --> G[20개 필드 기대 / 21개 필드 제공]
    G --> H[Out-of-Bounds Memory Read]
    H --> I[예외 처리 실패]
    I --> J[Windows BSOD]
```

여기서 중요한 점은 Rapid Response Content 자체가 일반적인 의미의 “새 센서 코드”나 “새 커널 드라이버”는 아니었다는 점입니다. 그러나 그 설정 데이터를 해석하는 센서가 운영체제 깊은 영역에서 동작하고 있었기 때문에, 설정 데이터의 형식 불일치가 단순한 애플리케이션 오류가 아니라 운영체제 크래시로 확산되었습니다.

즉, 이 사건의 핵심은 단순히 “업데이트 실수”가 아닙니다.

> 커널 수준 보안 제품에서는  
> 코드 변경이 아닌 설정 데이터의 오류도  
> 운영체제 전체 장애로 확산될 수 있다는 점입니다.
...
BYOVD 공격 흐름은 다음과 같이 단순화할 수 있습니다.

```mermaid
flowchart LR
    A[공격자] --> B[합법적으로 서명된 취약 드라이버 확보]
    B --> C[피해 시스템에 드라이버 로드 시도]
    C --> D[커널 권한 확보]
    D --> E[보안 제품 무력화]
    D --> F[프로세스/파일/메모리 은폐]
    D --> G[권한 상승 및 내부 장악]
...
아래 표는 제품의 우열을 단순 비교하기 위한 것이 아닙니다.  
보안 제품이 어느 정도 운영체제 깊은 영역에 의존하는지, 그리고 그로 인해 어떤 검증 질문이 필요한지를 정리하기 위한 표입니다.

특히 커널 모드 사용 여부는 벤더가 공개 문서에서 명확히 밝히지 않는 경우가 많습니다. 따라서 아래 표의 제3자 제품 평가는 **공개 자료와 제품 기능 구조를 기준으로 한 참고 판단**입니다. 실제 도입 시에는 벤더에 커널 드라이버 사용 여부, 드라이버명, 로드 조건, 업데이트 방식, 장애 시 복구 절차를 별도로 확인해야 합니다.

반면 PLURA-EDR은 구조가 명확합니다. Windows에서는 이벤트 채널 구독 모델로 동작하고, Linux에서는 `/var/log`의 각 로그 파일을 리스닝하는 방식으로 동작합니다. 따라서 Windows와 Linux 모두에서 커널 모드 드라이버 방식으로 동작하지 않습니다.

| 제품 | 제품 성격 | 공개자료 기준 커널 모드 판단 | 판단 근거 | 블로그용 표현 |
|---|---|---|---|---|
| **PLURA-EDR / PLURA-XDR** | 상세 로그·행위 기반 EDR, XDR 확장 구조 | **아니오 / 커널 모드 아님** | Windows는 이벤트 채널 구독 모델, Linux는 `/var/log` 각 파일의 리스닝 모드로 동작함. 두 OS 모두 커널 모드 드라이버 방식이 아니라, 운영체제와 웹에서 발생한 로그·증거를 수집해 상관분석하는 구조 | 커널 깊숙이 들어가는 감시자가 아니라, OS와 웹에서 발생한 증거를 수집·상관분석하는 구조 |
| **SentinelOne Singularity Endpoint** | NGAV + EDR/XDR 계열 엔드포인트 보호 제품 | **확인 필요 / 커널 의존 가능성 높음** | 차세대 AV·EDR 제품군 특성상 파일·프로세스·행위 차단, 자체 보호, 랜섬웨어 차단을 위해 드라이버 또는 낮은 수준의 OS 연동을 사용할 가능성이 있음. 단, 공개 문서만으로 제품별 드라이버 구조를 단정하기는 어려움 | 강력한 자율형 EDR일수록 커널 드라이버와 자체 보호 구조를 별도 확인해야 함 |
| **Elastic Defend** | Elastic Security 기반 EDR / Endpoint Protection | **확인 필요 / 일부 저수준 OS 연동 가능성** | 시스템 활동 데이터를 선별 수집하고, 드라이버 로드 시 스캔한다고 설명됨. 다만 공개 문서만으로 제품 자체 커널 드라이버 구조를 단정하기는 어려움 | 선별 수집형 EDR이며, 커널 드라이버 사용 여부는 별도 확인이 필요함 |
| **AhnLab EPP / EDR / V3 계열** | V3, 패치, 취약점 점검, 개인정보 보호, EDR을 묶은 EPP | **가능성 높음 / 확인 필요** | V3 안티멀웨어와 EDR 모듈을 포함하는 통합 엔드포인트 보호 플랫폼 구조. AV·안티멀웨어 계열은 일반적으로 파일 시스템 감시와 차단을 위해 드라이버를 사용할 가능성이 높음 | 통합 EPP 구조이므로 V3·EDR 모듈의 커널 드라이버 사용 여부를 반드시 확인해야 함 |
| **Genian Insights E** | EDR + AV + 안티랜섬 + 매체제어 통합 단말 보안 플랫폼 | **가능성 높음 / 공개자료상 명시 부족** | 단일 에이전트로 EDR, AV, 안티랜섬, 매체제어를 제공하고 파일·레지스트리·프로세스·DLL 행위를 수집한다고 설명됨 | 단말 행위 감시 범위가 넓기 때문에 커널 드라이버, 파일 필터, 자체 보호 방식 확인이 필요함 |
| **Genian NAC** | 네트워크 접근제어, 자산 식별, 엔드포인트 제어 | **커널 모드 중심 제품으로 보기 어려움 / 일부 기능 확인 필요** | NAC는 네트워크 접근제어, 자산관리, OS 정보 수집, 플러그인 기반 제어가 중심. 다만 인터페이스 제어, 장치제어 등 일부 기능은 높은 권한 또는 드라이버 사용 여부 확인 필요 | EDR처럼 모든 행위를 감시하는 제품이라기보다 네트워크 접근제어와 단말 상태 점검 중심으로 분리해 보는 것이 적절함 |

> 표의 판단은 제품의 우열을 확정하는 평가가 아닙니다. 특히 SentinelOne, Elastic Defend, AhnLab EPP/EDR/V3, Genian Insights E, Genian NAC의 커널 모드 사용 여부는 공개 문서만으로 단정하기 어려운 항목이 있습니다. 따라서 “확인 필요”, “가능성 높음”, “일부 기능 확인 필요”라는 표현은 공개자료 기준의 보수적 표현이며, 실제 도입·비교 단계에서는 벤더로부터 커널 드라이버 사용 여부와 드라이버명, 메모리 접근 범위, 업데이트·롤백·복구 절차를 공식 확인해야 합니다.

이 표에서 중요한 것은 특정 제품을 단순히 “위험하다”고 말하는 것이 아닙니다.

핵심은 다음입니다.  

> 보안 제품이 강력할수록 더 깊은 권한을 요구할 수 있습니다.  
> 그러나 더 깊은 권한은 더 강한 검증을 필요로 합니다.

따라서 앞으로 EDR, EPP, NAC 제품을 검토할 때는 탐지율만 볼 것이 아니라 다음 질문을 함께 던져야 합니다.

| 확인 질문 | 의미 |
|---|---|
| 커널 모드 드라이버를 사용하는가? | OS 전체 장애로 확산될 가능성 확인 |
| 드라이버명과 로드 조건은 무엇인가? | 실제 운영체제 침투 깊이 확인 |
| 프로세스 메모리를 읽거나 쓰는가? | 민감 정보 접근 가능성 확인 |
| 파일 시스템 필터 드라이버를 사용하는가? | 파일 접근·차단 과정의 안정성 확인 |
| 자체 보호 기능은 어떤 권한으로 동작하는가? | 보안 제품 자체가 새로운 공격면이 되는지 확인 |
| 업데이트는 단계적 배포와 롤백이 가능한가? | CrowdStrike와 같은 대규모 장애 가능성 완화 |
| 장애 시 안전모드·원격 복구 절차가 있는가? | 부팅 불가 상황 대응 가능성 확인 |
| 탐지 결과가 다른 로그와 상관분석되는가? | 단일 에이전트 판단에 의존하지 않는지 확인 |

결국 질문은 하나입니다.

> 이 제품은 얼마나 잘 탐지하는가?  
> 만이 아니라,  
> 이 제품은 얼마나 깊은 권한으로 동작하며,  
> 그 권한은 충분히 검증되고 통제되고 있는가?

---

## 10. 보안 제품도 감시와 검증의 대상이다
...
## 11. PLURA-XDR 관점: 단일 감시자가 아니라 증거 기반 상관 분석
...
* [CrowdStrike — Falcon Content Update Remediation and Guidance Hub: Channel File 291 Incident](https://www.crowdstrike.com/falcon-content-update-remediation-and-guidance-hub/)
...
* [Apple Developer — Deprecated Kernel Extensions and System Extension Alternatives](https://developer.apple.com/support/kernel-extensions/)
* [Elastic Docs — Event capture and Elastic Defend](https://www.elastic.co/docs/solutions/security/manage-elastic-defend/event-capture-elastic-defend)
* [Genian Insights E — 지니언스 통합 단말 보안 플랫폼](https://www.genians.co.kr/products/genian-insights-e)
* [Genian NAC Documentation — 엔드포인트 제어](https://docs.genians.com/release/ko/endpoint.html)
