...
특히 DB 암호화는 이 차이를 잘 보여줍니다.  
DB 암호화는 저장 매체, 백업 파일, DB 파일이 유출되었을 때 데이터를 보호하는 데 의미가 있습니다.  
그러나 웹 애플리케이션의 취약점을 이용한 SQL Injection 공격을 막는 것은 아닙니다.  
SQL Injection은 공격자가 애플리케이션의 정상 DB 조회 경로를 악용하는 공격이기 때문입니다.  
애플리케이션이 정상 권한으로 DB에 접근해 데이터를 조회하면, DB 암호화가 되어 있어도 결과 데이터는 애플리케이션을 통해 노출될 수 있습니다.
...
DB 암호화는 데이터가 저장된 상태를 보호하는 기술이지, 웹 애플리케이션을 통해 들어오는 SQL Injection 공격을 차단하는 기술은 아닙니다.
---
