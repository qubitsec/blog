...
“보안 솔루션은 많을수록 좋다”는 믿음으로 **다양한 보안 제품**(WAF, IDS, IPS, EDR, NDR 등)을 겹겹이 도입하는 경우가 있습니다. 하지만 복잡성이 커질수록, **전체 시스템의 동작 원리를 완전히 이해하는 사람이 아무도 없는** 상황이 발생합니다. 결국 **보안을 위한 설계가 오히려 보안의 사각지대를 만드는 아이러니**를 낳게 됩니다.

1. **솔루션 간 충돌·비효율**

   * 각 제품마다 **API 연동**이나 **로그 포맷**이 달라, 일관성 있는 관제가 어렵습니다.
   * **경보**(이벤트) 관리가 중복 또는 분산되어, 정작 **위험 징후**를 놓칠 가능성이 큽니다.
...
3. **알람 피로**(Alert Fatigue)
   ...
   * 서로 다른 솔루션이 동시에 ‘경보’를 쏟아내면, 보안 담당자는 **오탐**까지 모두 확인해야 해 **업무 과중**이 심해집니다.
   * 결국 실제 침입 시도(T1190, T1059.004 등)를 제때 대응하지 못할 위험이 커집니다.

> **결론**:
> 다중∙계층 보안 모델은 매우 신중하게 의사결정을 해야 합니다. 다중∙계층 보안 모델이 꼭 필요할 때도 있지만, 그 한계를 정확히 인지하고, 통합 솔루션으로 보완하는 것이 반드시 선행되어야 합니다.  
> **플랫폼 간 호환성**·**유지보수 비용**·**경보 피로**를 종합적으로 고려하여, **통합 플랫폼**(예: PLURA-XDR)을 도입해 **중복 기능**을 정리하고 **중심화된 관제**를 실현하는 것이 훨씬 효율적입니다.

이처럼 **보안 솔루션이 분산**되어 있으면, **일관된 관제**와 **즉각적인 대응**이 사실상 불가능하므로, 결과적으로 **APT 공격**의 초기 이벤트(웹셸 업로드·RCE 등)조차 놓칠 위험이 큽니다. 따라서, 보안팀이 한정된 인력·예산으로 **실질적인 방어** 역량을 갖추려면, 다중∙계층 보안보다는 ‘통합’을 우선 고려해야 합니다.  
