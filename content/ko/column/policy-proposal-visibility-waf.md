...
외부 구간에는 본질적으로 다음 트래픽이 대량 존재합니다.

* 인터넷 상시 스캔(포트 스캔, 취약점 스캔)
* 봇 트래픽/크롤러/오탐 유발 트래픽
* 방화벽·WAF에서 **이미 차단될 트래픽**
* 서비스와 무관한 비정상 세션(리트라이 폭주, 잘못된 요청 등)

이런 트래픽까지 전량 분석하면,
* “탐지 이벤트는 많지만” **대부분 차단된 것**이라 **실제 위험과 연결되지 않고**
* 분석자는 **내부로 유입된 공격(실제 피해 가능성)**에 집중하지 못해  
결국 **‘분석은 했는데 성과가 없는’ 결과**로 끝나기 쉽습니다.
