...
* **무엇보다도, VPN 장비는 반드시 ‘신뢰된 IP에서만 접속할 수 있도록’ 방화벽으로 제한해야 합니다.**
  단지 인증 방식이 아니라, **‘접근 가능성 자체를 제한하는 것’이 침해를 원천 차단할 수 있는 가장 확실한 방법**입니다.
  이 기본적인 설정만 적용했더라도, 많은 침해 사고는 애초에 발생하지 않았을 가능성이 높습니다.
...
원격 근무자 또는 재택 근무자의 접속 환경은 일반적인 VPN 설계와 다릅니다.  
특히 **IP 주소가 유동적**이거나 **외부 이동 중에 접속해야 하는 경우**,  
기존처럼 "신뢰된 고정 IP만 허용"하는 구조를 적용하기 어렵습니다.

하지만 그렇다고 해서 **IP 제어 없이 VPN을 무방비로 열어둘 수는 없습니다.**  
현실적인 방안은 다음과 같습니다:

---

### 6-1️⃣ 중요 서비스 운영 네트워크 접근 시: 반드시 IP 제한

운영 서버, 배포 시스템, DB 등 **중요 자산**에 접근해야 하는 사용자는:

- **지정된 장비, 고정망 환경에서만 접속**해야 합니다.  
  → 예: 사무실, 사설망, 전용 VDI 등
- **방화벽에서 IP 주소 명시 허용**  
  → VPN 포트 외에도 서비스 포트(IP 제한), 로그 저장 필수

> ✅ 중요 시스템은 "누가 접속했는가"가와 함께, "**어디에서 접속했는가**"가 중요합니다.

---

### 6-2️⃣ 일반 사내 시스템(VPN 연결 후 인트라넷) 접근 시: 네트워크 분리 필수

재택 근무자, 외부 협력사 등이 VPN을 통해 사내 리소스를 사용할 경우에도:

- **VPN 접속 구간과 실제 서비스 구간을 분리**  
  → 예: VPN 사용자는 DMZ까지만 접근, 운영망 차단
- **접근 가능한 시스템을 계정별로 제한**
- **VPN 연결 이후에도 서비스별 인증 이중화 적용**

> ✅ VPN 연결이 ‘모든 권한 허용’이 되어서는 안 됩니다.  
> ✅ **"VPN 연결 = 통로 확보", 그 후는 별도의 통제가 필요합니다.**

---

### 6-3️⃣ 사내 접근이 꼭 필요한가? SaaS 전환 검토

단순 업무 시스템(그룹웨어, 일정, 메일 등)은 **사내망에 묶어둘 필요가 없습니다.**

- Google Workspace, M365, Notion, Jira 등 **SaaS 기반 전환**
- 외부에서도 보안 인증(MFA 등)으로 사용 가능
- **VPN 없이도 안전하게 업무 가능**

> ✅ VPN을 복잡하게 관리하지 말고, **업무 자체를 클라우드로 이동**하는 것도 하나의 전략입니다.

---

### 📌 구조적 정리: 역할별 접속 방식 권고안
...
> 🔐 VPN만 보완하는 것이 아니라, **전체 접속 구조를 재설계해야 합니다.**
...
5. **방화벽을 통한 IP 접근 제한**: **지정된 신뢰 IP에서만 OpenVPN 포트에 접근 가능하도록 방화벽 정책을 구성해야 합니다.**
   → 이를 통해 **인증 이전의 공격 시도 자체를 원천 차단**할 수 있습니다.

> 🔺 **만약 불특정 다수가 접속해야 해 IP를 제한할 수 없다면, 그 환경에서는 SSL VPN 또는 OpenVPN을 사용하는 것이 적합하지 않습니다.**  
> 🔺**IP 접근 제어 없이 운영되는 VPN은 내부망을 ‘전면 인터넷에 개방’하는 것과 다르지 않으며, 구조적으로 침해를 초래할 수 있습니다.**
