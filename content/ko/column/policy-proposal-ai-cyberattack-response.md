...
### 6-5) 제로트러스트는 새로운 와일드파이어가 될 수 있다

Windows 환경에서 정상 관리 도구와 정상 기능은  
공격자에게 강력한 내부 확산 수단이 될 수 있다.

PowerShell, certutil, rundll32, mshta 같은 정상 도구는  
관리자에게는 유용하지만  
공격자에게도 유용하다.
...
정상적으로 운영될 때 제로트러스트는  
접근을 통제하고, 권한을 제한하고, 내부 이동을 어렵게 만든다.

그러나 공격자가 제로트러스트 제어면을 장악하면  
그 힘은 반대로 작동할 수 있다.

* 정상 접근 정책을 공격 경로로 바꾸고
* 정상 인증 흐름을 우회 경로로 바꾸고
* 정상 에이전트 배포 체계를 내부 확산 경로로 바꾸고
* 정상 관리자 콘솔을 공격 지휘 체계로 바꿀 수 있다

기존에도 공격자는 내부망의 핵심 운영 인프라를 장악해  
비즈니스 운영 자체를 멈추게 만들었다.

이랜드 랜섬웨어 사고에서는  
사내 네트워크 시스템이 공격받았고,  
NC백화점과 뉴코아아울렛 등 50개 점포 중 23개 점포 운영에 차질이 발생했다.

이 사례가 보여주는 것은 단순하다.

공격자는 서버 하나만 노리지 않는다.  
조직 운영을 지탱하는 인증, 정책, 배포, 결제, POS, 내부 시스템을 노린다.

미토스 시대에는 제로트러스트 정책 서버, IAM 서버, 관리자 콘솔, 에이전트 배포 서버도  
이와 같은 핵심 운영 인프라가 될 수 있다.

따라서 제로트러스트 제어면 침해는  
단순한 보안 제품 장애가 아니라  
조직 전체 접근 통제 체계의 붕괴로 이어질 수 있다.

참고: [Retail giant E-Land suspends operations at nearly half of stores due to ransomware attack](https://en.yna.co.kr/view/AEN20201122001900320)
