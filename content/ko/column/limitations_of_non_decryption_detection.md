...
### Note: 2026.07.03 — “공격의 맥락”이라는 표현의 문제

데일리시큐 쿼드마이너 인터뷰 제목은 “AI 보안의 핵심은 경보가 아니라 공격의 맥락을 보여주는 데이터”라고 되어 있다. 그러나 공격의 맥락을 보여주려면 단순한 패킷 외형이 아니라 payload, 애플리케이션 로그, 계정 행위, 엔드포인트 이벤트, 파일 및 요청 내용처럼 의미를 가진 데이터가 필요하다.

암호화된 내용을 보지 못한 채 패킷 크기·방향·순서·통신 간격·세션 흐름만 분석하는 것은 공격의 맥락을 보여주는 것이 아니다. 이는 통신의 외형적 정황을 바탕으로 이상 가능성을 추정하는 수준이며, “공격자가 무엇을 했는가”를 설명하는 증거가 아니라 “무언가 이상해 보인다”는 보조 단서에 가깝다.

AI에게도 마찬가지다. AI가 공격의 맥락을 설명하려면 최상의 데이터, 즉 실제 행위와 의미를 담은 데이터가 제공되어야 한다. payload, 로그, 계정 행위, 프로세스 실행, 파일 접근 기록 없이 패킷 크기·방향·순서·간격 같은 외형 정보만 제공하고 공격의 맥락을 설명하라고 하는 것은 기술이 아니라 마술에 가깝다. 모호한 데이터는 AI에게 맥락을 제공하는 것이 아니라, 사실이 아닌 내용을 그럴듯하게 제시하는 환각(Hallucination)을 발생시킬 뿐이다.

따라서 이를 “공격의 맥락을 보여주는 데이터”라고 표현하는 것은 기술의 실제 범위를 부풀릴 위험이 있다. 수박 껍질의 무늬와 두드린 소리만으로 수박의 맛을 알 수 없듯, 암호화된 payload를 해석하지 못하는 메타데이터 분석만으로 공격의 실체와 맥락을 설명할 수는 없다.

수박 겉핥기는 수박의 맛을 보여주지 못한다. 마찬가지로 패킷의 겉모습만 훑는 분석은 공격의 맥락을 보여주지 못한다.

---

### Note: 2026.07.10 — 샌즈랩 MNX·CTX와 “AI NDR” 주장에 대한 검토

샌즈랩도 AI 기반 네트워크 위협 탐지·대응(NDR) 솔루션 `MNX`와 사이버 위협 인텔리전스(CTI) 서비스 `CTX`를 연계해 위협 탐지, 분석, 대응 방향 도출을 제시하고 있다. 최근 보도에서는 MNX가 네트워크 보안 이벤트를 탐지하고, 암호화된 통신을 애플리케이션 단위로 구분하며, CTX와 연계해 악성코드 분석 및 위협 인텔리전스를 제공한다고 소개되었다.

그러나 이 역시 같은 기준으로 보아야 한다. NDR이 네트워크에서 관찰한 트래픽, 프로토콜, 세션, 파일, IoC, 이상 징후를 분석하는 것은 보조적 단서로 의미가 있다. 하지만 그것이 곧 공격의 실체와 맥락을 완전히 설명한다는 뜻은 아니다.

특히 암호화된 통신을 “애플리케이션 단위로 구분”하는 것과 암호화된 payload의 의미를 해석하는 것은 전혀 다른 문제다. 애플리케이션 식별은 “어떤 종류의 통신으로 보이는가”를 추정하는 것이지, “공격자가 무엇을 했는가”, “어떤 명령이 실행됐는가”, “어떤 데이터가 유출됐는가”를 입증하는 것이 아니다.

CTX 같은 위협 인텔리전스도 마찬가지다. CTI는 알려진 악성 IP, 도메인, 파일 해시, 공격자 그룹, 캠페인, 악성코드 행위 정보를 제공할 수 있다. 이는 탐지 결과를 풍부하게 설명하는 데 도움이 된다. 그러나 CTI는 고객 환경 내부에서 실제로 발생한 행위의 증거가 아니다. 외부 위협 정보는 내부 로그와 결합될 때 의미가 있으며, 그 자체만으로 침투 경로, 확산 범위, 데이터 유출 여부를 증명할 수는 없다.

AI에게도 최상의 데이터가 필요하다. payload, 웹 요청 본문, 애플리케이션 로그, 계정 행위, 엔드포인트 이벤트, 프로세스 실행, 파일 접근 기록 없이 네트워크 단서와 외부 CTI만 제공하고 공격의 맥락을 설명하라고 하는 것은 기술이 아니라 마술에 가깝다. 모호한 데이터는 AI에게 맥락을 제공하는 것이 아니라, 사실이 아닌 내용을 그럴듯하게 제시하는 환각(Hallucination)을 발생시킬 위험을 키울 뿐이다.

따라서 MNX·CTX 연계가 의미 없다는 뜻은 아니다. 네트워크 이상 징후 탐지, 알려진 위협 인텔리전스 매칭, 악성코드 샘플 분석, 대응 가이드 제공은 보안 운영에 도움이 될 수 있다. 문제는 이를 “공격의 맥락을 설명하는 AI NDR”처럼 과장하는 표현이다.

공격의 맥락은 네트워크의 겉모습에서 나오지 않는다. 공격의 맥락은 실제 행위가 기록된 로그와 증거에서 나온다. 수박 껍질의 무늬와 두드린 소리만으로 수박의 맛을 알 수 없듯, NDR이 관찰한 네트워크 정황과 CTI 보강만으로 공격의 실체를 모두 설명할 수는 없다.

결국 구매자가 물어야 할 질문은 단순하다.

* 암호화된 payload를 실제로 보는가?
* 웹 요청과 응답 본문을 확인할 수 있는가?
* 계정 행위와 엔드포인트 행위를 함께 연결하는가?
* 프로세스 실행, 권한 상승, 파일 접근 기록을 증거로 제시하는가?
* CTI 매칭이 아니라 실제 고객 환경에서 발생한 침해 흐름을 입증하는가?
* AI의 설명이 원본 로그와 증거에 근거하는가, 아니면 추정에 기반한 문장 생성인가?

이 질문에 답하지 못한다면, AI NDR이라는 표현은 보안의 본질을 설명하는 말이 아니라 또 하나의 마케팅 문장에 머물 수밖에 없다.
