...
암호화된 내용을 보지 못한 채 패킷 크기·방향·순서·통신 간격·세션 흐름만 분석하는 것은 공격의 맥락을 보여주는 것이 아니라, 통신의 외형적 정황을 바탕으로 이상 가능성을 추정하는 것이다. 이는 “공격자가 무엇을 했는가”를 설명하는 증거가 아니라 “무언가 이상해 보인다”는 보조 단서에 가깝다.

따라서 이를 “공격의 맥락을 보여주는 데이터”라고 표현하는 것은 기술의 실제 범위를 부풀릴 위험이 있다. 수박 껍질의 무늬와 두드린 소리만으로 수박의 맛을 설명할 수 없듯, 암호화된 payload를 해석하지 못하는 메타데이터 분석만으로 공격의 실체와 맥락을 설명할 수는 없다.
