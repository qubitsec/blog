...
1) **로그 수집 환경**부터 제대로 갖추세요.  
   ...
   - 수집해야 할 로그가 많아질수록 인프라(스토리지, 네트워크 대역폭) 부담이 커질 수 있으므로,  
     **우선순위**를 명확히 정하고 단계적으로 확장하는 접근이 바람직합니다.

2) **분석 기준과 탐지 룰**을 직접 설계하세요.  
   - 공격 시나리오를 구체화하고, **OWASP TOP 10**, **MITRE ATT&CK**이나 **내부 정책**을 참고해 룰을 작성해야 합니다.  
   - “어떤 이벤트를 이상 징후로 볼 것인가?” “어떤 로그를 우선 보관·분석할 것인가?” 등에 대한  
     **명확한 기준**이 없어서는 SIEM이 제대로 동작하기 어렵습니다.

3) **실무자의 이해와 교육**이 필수입니다.  
   - 아무리 좋은 SIEM이라도, **운영자가 로그와 룰을 모르면** 의미가 없습니다.  
   - 인력 양성, 교육, 보안팀과 개발팀 간 협업 체계를 정비하여, **실제로 운영 가능한 조직**을 만들어야 합니다.

4) **현실적인 운영 비용 증가**  
   - SIEM 도입 이후에는 **로그 스토리지**, **분석 인력**, **쿼리 튜닝** 등에 막대한 **운영 비용**이 필요합니다.  
   - 예를 들어, 본문(Body)나 OS 감사 로그처럼 **데이터 범위가 커지면** 스토리지 요구 사항이 급증하고, 
     수집되는 로그량이 증가하므로, **라이선스 비용**도 증가합니다.
     포렌식·분석 툴과 연동할 경우 로그량이 다시 추가되므로 추가 **라이선스 비용**이 발생할 수 있습니다.  
   - 또한 **로그 해석**을 제대로 하려면 **보안 전문 인력**(분석가, 엔지니어)도 필요하고,  
     이들이 **룰 업데이트**, **오탐/정탐 식별** 등에 시간을 투입해야 합니다.  
   - **솔루션 구매 비용**뿐 아니라, **장기적인 운영 비용**(스토리지, 인력, 유지보수, 라이선스 등)까지 고려해야  
...
필요하다면 **아카이빙(장기 보관)** 대상과 **온라인 분석** 대상을 구분해 보관하는 것이 좋습니다.  
또한, 인덱스 정책과 압축 기능을 활용해 **스토리지**를 효율적으로 관리할 수 있습니다.

**Q2.** “AI 모델만 적용하면 보안 인력이 줄어들까요?”  
**A.** AI 기반 분석은 **반복 업무**나 **오탐 필터링** 같은 부분을 자동화해줄 수 있지만,  
최종 해석(“이 이벤트가 정말 악성인지”)과 **룰 설계**는 여전히 **사람**이 주도해야 합니다.  
데이터 품질 관리와 **시나리오 업데이트** 등은 **전문 인력**이 필수입니다.

**Q3.** “우리 회사 공격 시나리오는 어떻게 만들 수 있나요?”  
**A.** **OWASP TOP 10**, **MITRE ATT&CK** 같은 **공개 프레임워크**를 참고하되,  
자체 서비스나 내부 업무 절차를 반영해 **맞춤형 룰**을 작성해야 합니다.  
주기적으로 **테스트 및 검증**을 진행하여, 실제 공격 가능성을 평가하고 룰을 고도화하세요.

아래는 **Q3** 대신 새로운 FAQ 항목으로, \*\*“SIEM을 구축하면 SOAR(자동 대응)가 가능한가요?”\*\*를 추가한 예시입니다.
이전 질문/답변의 흐름을 유지하면서, **SIEM 운영의 현실적 어려움**을 다시 강조하는 내용으로 구성했습니다.

**Q4.** “SIEM을 구축하면 SOAR(자동 대응)가 가능한가요?”
**A.** 이론적으로 **SOAR**(Security Orchestration, Automation, and Response)는 **SIEM**에서 발생한 경고와 이벤트를
**자동으로 처리**할 수 있는 **고도화된 솔루션**입니다.

* 예: 의심스러운 IP를 자동으로 방화벽 차단, 계정 공격 시도가 감지되면 자동 잠금 등
* 하지만, **SOAR**를 제대로 활용하려면

  1. **SIEM**에서 **정확한 이벤트**와 **정탐 알람**이 올라와야 하고,
  2. 그 알람에 대한 **자동화 규칙**(워크플로우)이 정교하게 설정되어 있어야 합니다.

지금까지 논의한 것처럼,

* **SIEM** 자체가 **로그 품질**, **분석 룰**, **운영 인력** 부족으로 인해 제대로 작동하지 않으면,
* **SOAR**로 넘어가도 **오탐이 자동으로 실행**되거나, **잘못된 차단**이 발생하는 등 **심각한 운영 리스크**가 생길 수 있습니다.
