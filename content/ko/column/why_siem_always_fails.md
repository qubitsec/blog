...
아래는 2번 섹션(로그 분석? 할 수 없습니다)의 **세 가지 하위 항목**을
좀 더 **일반 독자도 쉽게 이해할 수 있도록** 풀어서 설명한 확장 버전입니다.
(예: 왜 Request Body가 필요한지, 왜 커널 레벨 백도어가 일반 로그로는 안 잡히는지,
DLL 인젝션 판별 시 사용자 정의 룰이 필요한 이유 등을 좀 더 상세히 써두었습니다.)
...
* 우선 **악성 페이로드**가 들어 있는 **필드**(주로 **Request Body**)를
...
* 물론, \*\*WAF(Web Application Firewall)\*\*나 별도 **에이전트**를 통해
...
* **커널 레벨**에서 동작하는 악성 코드(백도어)는,
  ...
  * 운영체제의 **일반 이벤트**(예: 프로세스 시작/중지, 네트워크 연결)만 보고는
    흔적을 잡아내기 힘듭니다.
