...
> 물론 일부 보안 솔루션(WAF 등)이나 고급 로깅 기능을 통해 **요청 본문과 응답 본문**을 캡처할 수 있습니다.  
> 그러나 성능 저하, 추가 비용 문제 등으로 인해 **대부분의 기업 환경에서 충분히 적용되지 못하는** 실정입니다.

### (2) 감사 정책 없는 운영체제
- Windows(윈도우)나 Linux(리눅스) 등 **운영체제 감사**(Auditing) 정책을 활성화하지 않으면  
  - 사용자 로그인, 프로세스 실행, 권한 변경 등의 이벤트가 **아예 기록되지 않습니다.**
- 감사 정책을 설정했다고 해도, **최소 수십만 건에서 수백만 건 이상의 로그**가 쌓이는 상황에서
  - **무엇이 정상**이고 **무엇이 이상**인지 판단할 **분석 기준**이 없다면 무용지물입니다.  
...
1. **단순 실패 로그만**으로는 공격 여부 판단이 어렵습니다.

   * 보통 로그인 실패·성공, IP 주소 정도는 남지만,
     “**이 시도가 유출된 비밀번호를 사용한 것인지**”는 알기 어렵습니다.
2. **반복·광범위 시도**를 잡아내려면 **사용자**가 **특정 조건**을 정의해야 합니다.

   * 예: “**같은 IP**에서 **짧은 시간** 안에 **여러 계정** 시도 발생” → 이상 징후로 간주
   * “**동일 계정**에 대해 10회 이상 로그인 실패” → 계정 잠금 혹은 경고
   * 이런 규칙을 **SIEM**에 **직접** 설정하지 않으면, 단순 실패 로그만 쌓일 뿐입니다.
3. \*\*유저 행태 분석(Behavior Analysis)\*\*이 필요합니다.

   * 크리덴셜 스터핑은 소수의 계정에 집중하기도 하고, 여러 IP·여러 계정을 동시다발로 시도하기도 합니다.
   * **SIEM**은 수집된 **로그**를 보여줄 뿐, “어떤 패턴이 공격인지”는 **사용자**가 정의해야 합니다.

#### 예시: “Credential Stuffing” 탐지 룰 만들기

* `WHERE action=’login’ AND result=’fail’ AND ip_count > 5 IN 10min`
* `WHERE username=X AND fail_count > 10 IN 1h`
* **사용자**가 이런 식으로 **시간·횟수** 기준을 정해 **조건부 알람**을 설정해야,
  SIEM이 “크리덴셜 스터핑 의심”이라고 알려줄 수 있습니다.

#### 결론

* **크리덴셜 스터핑** 방어에도 **로그**가 필수지만,

  * **IP·계정별 시도 횟수** 같은 **구체적 조건**을 마련하지 않으면,
  * SIEM은 단순 “로그인 실패가 좀 많네…” 정도로만 인식하게 됩니다.
* 고급 **자동 대응**(잠금, 캡차 요청, 추가 인증)까지 하려면,

  * **SOAR**나 **WAF** 등과 **연동**하고,
  * **사용자 행태 분석**(UEBA, User and Entity Behavior Analytics)까지 고려해야 합니다.
* 결국, **사용자**가 **룰**과 **탐지 로직**을 설계하지 않으면
  “크리덴셜 스터핑”이라는 공격도 **SIEM**만으로 자동 인식하기는 어렵습니다.
