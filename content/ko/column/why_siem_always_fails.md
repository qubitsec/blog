...
### 2.1 웹로그에서 공격 쿼리 추출 가능할까?

* SIEM에서 SQL Injection이나 XSS, RCE(원격 코드 실행) 같은 공격을 찾으려면

  * **악성 페이로드가 담긴 필드**를 파싱하고 인덱싱해야 합니다.
* 그러나 현실은 대부분의 SIEM이 **단순 URL, 상태 코드** 정도만 저장합니다.

  * “`WHERE uri CONTAINS 'select * from'`” 같은 쿼리 자체를 수행할 데이터가 **없습니다.**

아래 예시는, **“외부 도구를 통해 어떻게든 BPFDoor나 DLL 인젝션 관련 로그를 SIEM에 수집한다 해도,
사용자가 ‘이게 BPFDoor다’, ‘이게 DLL 인젝션이다’라고 스스로 검색해내야 하는 문제를 해결할 수 있을까?”**
라는 의문에 집중해 **한계점**을 좀 더 구체적으로 설명한 버전입니다.
