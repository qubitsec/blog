...
tags: ["동형암호", "FHE", "PQC", "격자", "LWE", "ML-KEM", "양자컴퓨터", "암호수학"]
---
...
NIST도 ML-KEM의 보안성이 Module-LWE의 계산 난이도와 관련되어 있으며,  
현재는 양자컴퓨터를 가진 공격자에 대해서도 안전한 것으로 여겨진다고 설명합니다. :contentReference[oaicite:0]{index=0}
...
NIST는 PQC FAQ와 표준 문서에서  
격자 기반과 해시 기반 문제가  
양자컴퓨터와 고전 컴퓨터 모두에게 어려운 문제로 여겨진다고 설명합니다.  
또 FIPS 203은 ML-KEM이 Module-LWE의 난이도에 기반하며  
현재는 양자 공격자에 대해서도 안전하다고 적고 있습니다. :contentReference[oaicite:1]{index=1}
...
NIST는 초기 PQC 표준 4개 중 3개가 structured lattices 계열이라고 설명합니다.  
이건 우연이 아닙니다.  
격자 계열은 현재까지 알려진 후보 중  
**일반 목적 공개키 암호로서 효율성이 매우 좋기 때문**입니다. :contentReference[oaicite:2]{index=2}
...
예를 들어 ML-KEM은  
키 교환용 PQC로서 실전 배치가 가능한 수준의 성능을 제공했고,  
그래서 NIST의 첫 KEM 표준이 되었습니다.  
NIST SP 800-227도 ML-KEM을 NIST 승인 lattice-based KEM으로 다루고 있습니다. :contentReference[oaicite:3]{index=3}

즉, 격자는  
“아직 안 깨졌으니 대충 쓰자”가 아니라  
**실제로 써도 될 만큼 빠르고 작고 유연한 후보**였기 때문에 살아남았습니다.
...
NIST의 PQC 관련 발표 자료도  
격자에 대해서는 연구가 많고,  
많은 암호 기능을 lattice 위에서 구현할 수 있다고 설명합니다. :contentReference[oaicite:4]{index=4}
...
NIST도 PQC 경쟁 과정에서  
미래 양자 알고리즘이 새로 발견될 가능성과,  
미래 양자컴퓨터의 비용·속도·메모리 같은 불확실성을  
명시적으로 언급해 왔습니다. :contentReference[oaicite:5]{index=5}
...
NIST도 HQC를 2025년에 다섯 번째 PQC 알고리즘으로 추가 선정하며,  
이는 ML-KEM이 언젠가 깨질 가능성에 대비한 **backup defense** 성격이라고 설명했습니다. :contentReference[oaicite:6]{index=6}

이 사실은 매우 중요합니다.

즉,

- NIST도 lattice 하나에 모든 것을 걸지 않고 있고
- backup KEM을 함께 준비하고 있으며
- 미래 공격, 특히 새로운 양자 알고리즘 가능성을 완전히 배제하지 않습니다

이 점에서  
격자를 쓰는 이유는 **확신의 100%**가 아니라  
**현재까지의 최선의 균형과 최선의 분석 결과**입니다.

---

## 동형암호와 PQC가 모두 격자를 쓰는 것이 오히려 주는 메시지
...
- 그래서 NIST도 lattice 외 계열 후보를 일정 부분 유지하려 합니다
...
이 점이  
최근 NIST가 “추가 general-purpose signature는 structured lattice가 아닌 쪽에도 관심이 있다”고 말하는 배경과도 맞닿아 있습니다. :contentReference[oaicite:7]{index=7}
...
격자가 강한 이유를 이해하는 것만큼 중요한 것은  
**격자 하나에만 영원히 기대지 않는 태도**입니다.

2026년 현재의 현실은 이렇습니다.

- 격자는 가장 강력한 주류 후보이고
- 동형암호와 PQC 모두 그 위에 크게 기대고 있으며
- 그러나 NIST조차 backup과 다변화를 동시에 준비하고 있습니다
