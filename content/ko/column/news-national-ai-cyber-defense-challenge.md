...
- **정보보안**은 정보와 업무 체계를 안전하게 관리하는 큰 틀입니다.
- **IT 통제**는 사용자, 권한, 문서, DB, 자산, 설정, 증적을 정책에 맞게 관리하는 영역입니다.
- **사이버보안**은 공격자의 침투 행위를 실시간으로 탐지·차단·분석·대응하는 영역입니다.
- **ISMS-P 인증은 필요하지만 면책이 아닙니다.** 로그 보관과 인증 대응만으로는 실시간 침해 대응 능력을 증명할 수 없습니다.
...
이 글의 핵심은 정보보안, IT 통제, 사이버보안을 대립시키자는 것이 아닙니다.  
각각의 역할을 정확히 구분하자는 것입니다.
...
아래 표는 세부 항목이 많으므로 필요할 때 펼쳐 보는 참고 목록으로 두었습니다.

<details>
<summary><strong>운영·관리·컴플라이언스 통제 세부 항목 보기</strong></summary>
...
다만 ISMS-P는 기본적으로 **정보보안 관리체계 인증**입니다.  
따라서 실시간 공격 탐지, 상관분석, 차단, 침해 흐름 재구성 능력을 직접 평가하는 체계로 보기는 어렵습니다.

### 3-1. ISMS-P가 주로 확인하는 것

ISMS-P 안에도 사이버보안과 관련된 통제 항목은 있습니다.
...
### 3-2. 로그 보관은 컴플라이언스이고, 로그 분석은 사이버보안입니다

ISMS-P에서 로그는 주로 **수집, 보관, 점검, 증적**의 관점으로 다루어집니다.  
예를 들어 6개월, 1년, 2년처럼 보관기간을 충족하는 일은 컴플라이언스에서 중요합니다.

하지만 로그를 오래 보관한다고 해서 공격을 실시간으로 막을 수 있는 것은 아닙니다.
...
### 3-3. 컴플라이언스는 두 축으로 나누어 보아야 합니다

앞으로의 컴플라이언스는 하나로 뭉뚱그릴 것이 아니라,  
...
다만 이 책임은 단순히 사고가 발생했다는 이유만으로 묻는 책임이 아닙니다.  
다음 질문에 답하지 못했을 때 발생하는 **운영 책임**입니다.
...
### 3-5. ISMS-P는 출발점이지 종착점이 아닙니다

ISMS-P를 만든 사람들이 잘못했다는 뜻은 아닙니다.  
...
문서 암호화, DRM, DLP, NAC, DB 암호화, DB 접근제어, 데이터 마스킹, SAST, SBOM, ISMS-P는 모두 중요합니다.  
그러나 이들은 주로 **사용자, 정보, 접근 권한, 저장 데이터, 구성요소, 증적**을 통제합니다.

반면 사이버보안 도구는 **공격자와 공격 행위**를 봐야 합니다.

차이는 다음 예시에서 분명해집니다.

| 정보보안·IT 통제 항목 | 할 수 있는 일 | 그것만으로 어려운 일 |
|---|---|---|
| 문서 암호화·DRM | 문서 열람과 반출 통제 | 웹셸 업로드 공격 탐지 |
| DLP | 데이터 반출 제한 | SQL Injection 실시간 분석 |
| NAC | 네트워크 접속 통제 | 크리덴셜 스터핑의 계정 행위 추적 |
| DB 암호화·DB 접근제어 | 저장 데이터 보호와 DB 접근 정책 집행 | 애플리케이션 계정 탈취 이후의 전체 침투 흐름 재구성 |
| SAST | 배포 전 소스코드 취약점 점검 | 운영 중인 웹 요청 공격 페이로드 차단 |
| SBOM | 소프트웨어 구성요소 식별 | 취약 구성요소가 실제 악용되는 순간 탐지 |
| ISMS-P | 관리체계와 증적 점검 | 실시간 공격 상관분석과 즉시 차단 |
...
## 5. “공격은 웹만 있는 것이 아니다”라는 말은 맞지만, 논점을 흐리면 안 됩니다
...
그리고 웹에서 시작된 공격은 보통 웹에서 끝나지 않습니다.
...
```text
[웹 요청] → [로그인/계정] → [서버 실행] → [PC·내부 이동] → [데이터 유출] → [차단·포렌식]
   WAF          IAM/AD          EDR/Sysmon        EDR/XDR             DLP/Proxy        XDR/SIEM
```
...
예를 들어 제로트러스트 관점에서는  
IAM, MFA, PAM 같은 계정·권한 통제 영역이 매우 중요합니다.
...
이런 흐름을 하나의 맥락으로 이어보기 위해 XDR 모델이 중요해지고 있습니다.

예를 들어 PLURA-XDR은 이런 철학을 바탕으로 통합 보안 플랫폼을 지향합니다.  
중요한 것은 특정 제품 이름 자체가 아니라, 무엇을 가능하게 하느냐입니다.
...
인증서는 공격을 막지 못합니다.  
로그를 보는 능력, 공격을 연결하는 능력, 즉시 대응하는 능력이 공격을 줄입니다.
