...
Chrome/Chromium 진영은  
양자 위협 대응에서  
가장 시급한 축을 **HTTPS의 키 교환 보호**로 보고 있으며,  
웹 PKI의 인증서 민첩성까지 함께 강조하고 있습니다.

실제로 Chromium은  
Chrome 124부터 데스크톱 플랫폼에서  
TLS 1.3과 QUIC에 대해  
최신 Kyber 계열 하이브리드 키 교환을 기본 활성화했다고 밝혔습니다.

Firefox도  
132 버전에서  
TLS 1.3용 **`mlkem768x25519`**  
포스트 양자 키 교환 지원을 추가했습니다.

Apple 역시  
iOS 26, iPadOS 26, macOS Tahoe 26, visionOS 26에서  
TLS 1.3 하이브리드 양자 안전 키 교환 지원을 자동 광고하도록 준비하고 있으며,  
서버 측에서 `X25519MLKEM768`을 지원하면 이를 협상하도록 안내하고 있습니다.

이 대목이 중요합니다.

브라우저는 단순히  
“OpenSSL이 지원하니 우리도 켠다” 식으로 움직이지 않습니다.  
브라우저마다 자체 TLS/암호 구현체와 정책이 다르기 때문입니다.
...
IETF에서도  
TLS 1.3 하이브리드 키 교환과  
ML-KEM 기반 TLS 그룹 표준화가 계속 진행되고 있습니다.
