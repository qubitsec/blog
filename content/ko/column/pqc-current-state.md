...
브라우저는 단순히  
“OpenSSL이 지원하니 우리도 켠다” 식으로 움직이지 않습니다.  
각자 사용하는 TLS/암호 구현체와 정책이 다르기 때문입니다.

- Chrome 계열은 주로 **BoringSSL**
- Firefox 계열은 주로 **NSS**
- Safari 계열은 **Apple 플랫폼 보안 스택**
- Windows 애플리케이션 다수는 **SChannel/CNG**
- 일부 서버와 애플리케이션은 **OpenSSL**

즉,  
**OpenSSL 지원 = 전체 브라우저 지원**은 아닙니다.
...
2. 프로토콜 협상 방식을 정하고  
3. 인증서와 서명 체계를 검증하고  
4. 성능 및 호환성 문제를 테스트한 뒤  
5. 점진적으로 기본값에 반영하는 순서로 움직입니다.

그래서 현재 단계의 핵심은  
“PQC가 이미 널리 완전히 켜졌다”가 아니라,  
**주요 브라우저와 애플리케이션이 하이브리드 키 교환부터 점진적으로 실제 배포를 시작했다**는 데 있습니다.

결국 실무적으로는 이렇게 이해하는 것이 맞습니다.

> **OpenSSL의 PQC 지원은 “준비 완료”가 아니라  
> 애플리케이션과 브라우저가 따라올 수 있는 기반이 마련됐다는 의미에 가깝습니다.**

그리고 진짜 활성화의 관건은  
라이브러리 하나가 아니라  
**브라우저, CA, 운영체제, 네트워크 장비, 기업 보안 인프라가 함께 움직일 수 있느냐**에 달려 있습니다.
...
실무자는 종종  
OpenSSL 지원 여부만 보고  
“이제 서비스 적용이 가능하겠다”고 생각합니다.

하지만 실제로는  
클라이언트 애플리케이션과 브라우저가  
어떤 TLS 스택을 쓰는지,  
하이브리드 키 교환을 지원하는지,  
인증서와 서명 알고리즘을 어디까지 받아들이는지  
별도로 확인해야 합니다.

특히 공개 웹 서비스라면  
서버 한쪽만 PQC를 지원한다고 끝나지 않습니다.
...
즉, PQC 전환은  
서버 라이브러리 업그레이드 프로젝트가 아니라,  
**클라이언트까지 포함한 전체 서비스 체계의 마이그레이션 과제**입니다.
...
ARIA 자체가 기술적으로 의미 없었다는 뜻은 아닙니다.  
오히려 국내 암호 기술을 확보하고,  
국가 차원의 표준과 구현 역량을 축적했다는 점에서  
분명 중요한 성과가 있었습니다.
...
브라우저, 운영체제, TLS 라이브러리, HSM, 클라우드, 보안장비,  
글로벌 서비스 인프라까지 함께 움직이지 않으면  
국내 중심 채택은 유지 비용과 호환성 부담만 키울 수 있습니다.

즉, ARIA 사례가 오늘 PQC 논의에 주는 교훈은 분명합니다.

> **국산 암호를 만든 것 자체가 목적이 되어서는 안 된다.  
> 진짜 목표는 국제 표준과 연결되고,  
> 실제 운영 생태계에서 살아남을 수 있는 상호운용성을 확보하는 것이다.**

PQC는 ARIA 때보다 훨씬 더  
브라우저, 앱, 인증서, 클라우드, 반도체, 네트워크 장비까지  
글로벌 연동성이 중요한 분야입니다.

그래서 한국형 PQC가  
과거의 한계를 반복하지 않으려면  
“국내 채택”보다 먼저  
**국제 호환성과 생태계 확장성**을 기준으로 설계되어야 합니다.
...
하지만 그 목표가  
“한국만 쓰는 독자 암호 채택”이 된다면,  
그것은 사실상 **ARIA의 전철을 다시 밟는 길**에 가깝습니다.
...
| **FN-DSA** | 전자서명 | 성능 장점으로 주목받지만 표준화 진행 중 | 당장 운영 기본값보다 관찰 대상에 가까움 |
