...
보수적 대안의 의미를 가집니다. :contentReference[oaicite:6]{index=6}
...
이 버전은 2025년 4월 공개된 LTS 버전입니다.** :contentReference[oaicite:7]{index=7}
...
이 흐름은 표준 확정 알고리즘이 OpenSSL 본체로 흡수되는 방향과 맞물리고 있습니다. :contentReference[oaicite:8]{index=8}
...
3. 앞으로는 **NIST 표준 중심의 운영 구현**으로 수렴할 가능성이 높다. :contentReference[oaicite:9]{index=9}
...
- **Chrome 131**은 최종 ML-KEM 기반 하이브리드 키 교환으로 전환했습니다. :contentReference[oaicite:10]{index=10}
- **Firefox 135**도 ML-KEM 기반 지원 흐름에 들어섰습니다. :contentReference[oaicite:11]{index=11}
- Apple은 **iOS 26 / macOS Tahoe 26** 계열에서 `X25519MLKEM768` 지원을 문서화했습니다. :contentReference[oaicite:12]{index=12}
...
**10.0(2025년 4월)** 에서 이를 새로운 기본 키 교환으로 지정했다고 설명합니다. :contentReference[oaicite:13]{index=13}
...
사실상 **OpenSSL 3.5 이상 환경**이 필요하다고 설명합니다. :contentReference[oaicite:14]{index=14}

또 NGINX 공식 릴리스 문서는  
**NGINX Plus R33** 에 initial support for Post Quantum Cryptography가 추가됐다고 명시합니다. :contentReference[oaicite:15]{index=15}
...
2025년 10월에는 HAProxy GitHub에 OpenSSL 3.5 기반 PQC 지원 요청 이슈가 올라왔습니다. :contentReference[oaicite:16]{index=16}
...
쿠버네티스 내부 전환의 복잡성을 설명합니다. :contentReference[oaicite:17]{index=17}

또 2026년 1월에는  
GitHub 이슈 `#136346`에서  
**PQC readiness observations**가 커뮤니티 차원에서 제기됐습니다. :contentReference[oaicite:18]{index=18}
...
ML-KEM과 ML-DSA 기반 PQC API가 일반 제공(GA) 단계에 들어갔다고 발표했습니다. :contentReference[oaicite:19]{index=19}
...
PQC와 하이브리드 전환의 필요성을 강조합니다. :contentReference[oaicite:20]{index=20}
...
ML-KEM에 대한 backup defense 성격을 분명히 했습니다. :contentReference[oaicite:21]{index=21}
...
OpenSSL도 이를 주류 구현으로 받아들이기 시작했습니다. :contentReference[oaicite:22]{index=22}
...
그리고 기업 내부 인프라가  
함께 상호운용성을 확보해야만 가능해집니다. :contentReference[oaicite:23]{index=23}
