...
> **PQC는 혁명이라기보다 진화입니다.**  
> 전체 암호체계를 새로 만드는 것이 아니라, 국제 표준에 맞춰 공개키 영역을 점진적으로 전환하는 문제입니다.
...
다만 양자컴퓨터가 대칭키 암호에 아무 영향도 없다는 뜻은 아닙니다.  
Grover 알고리즘 관점에서는 대칭키 탐색 비용이 줄어들 수 있으므로, AES-128보다 **AES-256을 사용하는 방향**이 더 충분한 안전 여유를 제공합니다.  
그러나 이것은 알고리즘 전체를 PQC로 교체한다는 의미가 아니라, 대칭키 영역에서는 대체로 **키 길이 상향과 안전한 운영 설정**으로 대응할 수 있다는 의미에 가깝습니다.
...
- 키 크기와 메시지 크기 증가
- TLS 핸드셰이크 패킷 증가
- MTU(Maximum Transmission Unit) 제한에 따른 패킷 분할(Fragmentation) 가능성
- 구형 방화벽·IPS·프록시·SSL 가시성 장비의 비정상 차단 가능성
- 저사양 장비 성능 영향
- HSM/KMS 지원 여부
- 인증서 체계와 장기검증 문제
...
특히 PQC 키 교환은 기존 ECC 기반 키 교환보다 메시지 크기가 커질 수 있으므로, 일부 네트워크에서는 TLS 핸드셰이크가 여러 패킷으로 나뉘거나 구형 보안장비가 이를 비정상 트래픽으로 오인할 수 있습니다.  
따라서 실제 운영에서는 암호 알고리즘 자체보다 **중간 장비 호환성, MTU, 패킷 분할, fallback 정책**을 함께 확인하는 것이 중요합니다.
...
- 대칭키 영역은 주로 AES-256처럼 충분한 키 길이를 쓰는 문제에 가깝다
