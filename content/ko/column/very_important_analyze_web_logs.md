...
### 4) 파라메터 변조 공격(Parameter Tampering) ✏️
...
새로운 취약점이 발견되면, 보안 룰 업데이트 전에 공격이 발생할 가능성이 높으므로 **주기적인 전체 로그 분석**으로 징후를 찾아내야 합니다.
...
### 12) 웹 서버 에러 분석 및 장애 원인 추적 🩺
웹 서버에서 발생하는 5xx 에러, 어플리케이션 충돌 로그 등을 **종합적으로** 분석합니다.
...
보안 문제와 운영 문제를 동시에 진단할 수 있어, **전체 로깅·모니터링 인프라**가 매우 중요합니다.

### 13) 공격 페이로드 우회 기법 감지(WAF Bypass 시도) 👀
...
이런 우회 기법은 개별 요청만 보면 정당한 접속처럼 보이기도 하므로, **종합적인 전체 로그 분석**이 필수입니다.

### 14) 이상 트래픽 시간대 분석 및 동적 방어 정책 생성 🕒
...
이상 트래픽 패턴을 **전체 로그** 분석하면, 공격 발생 전에 조치하여 **사전 방어**가 가능합니다.
