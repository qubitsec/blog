...
flowchart LR
    ...
    B --> C["🌐<br/><b>정상처럼 보이는<br/>API 요청</b>"]
    C --> D{"🔐<br/><b>객체 단위<br/>권한 검증</b>"}
    D -- "검증 미흡" --> E["🗄️<br/><b>DB 조회</b>"]
    E --> F["📄<br/><b>응답본문</b><br/>개인정보 포함"]
    F --> G["🚨<br/><b>정보 유출</b>"]
...
이번 사건은 “로그인 실패가 많았는가?”만으로 설명하기 어렵습니다.
...
| XDR    | 반복 호출 상관분석                 | 동일 IP·계정의 순차 ID 접근, 대량 조회 행위 분석        |
| SOAR   | 자동 차단                      | 위험 임계치 초과 시 IP, 세션, 계정 단위 차단           |
| 감사/조사  | 상세 로그 보존                   | 사고 원인 분석과 재발 방지를 위한 증적 확보              |

정리하면, API 정보 유출 방어의 핵심은 다음 네 가지입니다.
