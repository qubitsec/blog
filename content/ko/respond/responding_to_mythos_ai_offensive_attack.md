...
더 강하게 말하면,  
관리·운영·원격 접속 경로는  
정해진 IP 주소를 제외하고 원칙적으로 전면 차단해야 합니다.
...
| 3 | SSH/RDP/VPN/SSLVPN/OpenVPN | 인터넷 전체 공개 금지, 지정 IP와 승인된 경로만 허용 |
| 4 | 메일·협업·파일 전송 | 내부 서버 직접 노출 최소화, SaaS 분리 검토 |
| 5 | 방화벽·UTM·라우터·스위치 관리 콘솔 | 외부 공개 금지, 관리망 또는 지정 IP에서만 접근 |
| 6 | 클라우드 관리자 콘솔 | MFA, 관리자 분리, 접속 IP 제한, 감사 로그 필수 |
| 7 | 내부 관리자 페이지 | 외부 노출 금지, 접근 경로 분리 |
...
그러나 Mythos 시대의 1순위 공격면은 반드시 웹/API입니다.

웹/API를 보지 못하면  
제로데이 대응은 시작조차 할 수 없습니다.

특히 SQL Injection은 과거의 낡은 공격이 아닙니다.  
AI에 의해 다시 무기화될 수 있는 대표적인 웹 공격입니다.

Mythos급 AI가 SQL Injection 가능 지점을 자동으로 찾고,  
필터를 우회하고,  
응답 차이를 분석하며,  
공격 성공률을 극단적으로 높일 수 있다면  
전 세계 웹 서비스는 영화 속 제로데이가 현실이 되는 상황을 마주할 수 있습니다.

동시에 공격면은 웹/API에만 머물지 않습니다.

SSH, RDP, VPN, SSLVPN, OpenVPN, 메일 서버, 협업 시스템, 파일 전송 시스템,  
방화벽, UTM, 라우터, 스위치, 보안 장비 관리 콘솔까지  
외부에 노출된 모든 소프트웨어가 공격면이 됩니다.

따라서 대응 원칙은 명확합니다.

- 웹/API는 반드시 1순위로 보고 실시간 분석해야 합니다.
- SQL Injection은 AI 시대에 다시 무기화될 수 있는 핵심 위협으로 봐야 합니다.
- 외부에 공개할 필요가 없는 서비스는 닫아야 합니다.
- 정해진 IP 주소 외에는 원칙적으로 차단해야 합니다.
