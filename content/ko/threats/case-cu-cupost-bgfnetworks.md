---
title: "CUPOST 개인정보 유출 사고: 웹 취약점 침해와 CI 유출의 위험"
date: 2026-06-06
draft: false
...
| **PLURA-WAF** | 인증 토큰, 쿠키, 세션 | 인증 우회·세션 악용 여부 확인 |
| **PLURA-WAF** | 응답 본문, 응답 크기 | 개인정보 대량 응답 여부 확인 |
| **PLURA-WAF** | 세션·IP·User-Agent | 자동화된 대량 조회 여부 확인 |
| **PLURA-WAF** | API 객체 ID, 페이지네이션 | IDOR/BOLA/API 권한 검증 실패 여부 확인 |
| **PLURA-EDR** | 웹셸, 프로세스, 명령 실행 | 웹 취약점이 서버 침해로 이어졌는지 확인 |
| **PLURA-EDR** | 파일, 압축, 전송 | 개인정보 파일 생성·압축·외부 전송 확인 |
| **PLURA-EDR** | DB 접속 도구, 클라우드 CLI | 운영 서버에서 직접 추출했는지 확인 |
| **PLURA-XDR** | WAF + EDR + DB + Cloud 로그 상관 | SQLi, API 권한 실패, 웹셸, 계정 탈취 중 실제 경로 규명 |
| **PLURA-XDR** | 개인정보 응답·파일·전송 흐름 | 유출 범위와 실제 반출량 산정 |
