...
```mermaid
flowchart TD
    A[퇴사자: JWT 서명키 접근 가능] --> B[서명키 회수·갱신 누락]
    B --> C[공격자(해외 VPS)에서 JWT 무제한 생성]
    C --> D[사용자 ID: 1,2,3… 순차 증가로 반복 호출]
    D --> E[정상 로그인 없이 개인정보 조회]
    E --> F[5개월간 대규모 유출 지속]
    F --> G[총 3,370만 계정 정보 탈취]
````

---

## 6. PLURA-XDR / WAF가 막을 수 있었던 지점
