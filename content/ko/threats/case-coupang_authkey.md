...
- 정부 발표: 공격자는 **쿠팡 내부 인증에 사용되던 JWT 서명키**(signing key)를 이용하여**“정상 로그인 절차 없이도” 서버가 신뢰하는 JWT 토큰을 직접 생성**한 것으로 파악됩니다.
...
> [영상 요약]
> 
> 1. **공격 발생:** 공격자가 탈취한 JWT로 인증을 통과한 후, 스크립트를 이용해 수백 번의 API 요청을 전송합니다. (F12 네트워크 탭 확인)
...
하지만 **PLURA-XDR**은 인증을 넘어 **"인증된 사용자가 왜 1초에 100번이나 조회를 시도하는가?"**라는 **행위의 맥락**(Context)을 분석합니다. 이것이 바로 키 유출 사고가 발생해도 데이터를 안전하게 지킬 수 있는 핵심 이유입니다.
