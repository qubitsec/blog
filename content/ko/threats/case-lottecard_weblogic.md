...
> **판정:** 현재(2025) 기준으로 **LLM/에이전트 조합을 활용한 WAF 우회용 SOAP/XML 페이로드의 자동 생성·변형은 충분히 실현 가능한 공격 역량**입니다. 본 사건의 침투·유출은 **미패치된 WebLogic(CVE-2017-10271)** 과 **WAF의 본문/정규화 미적용·탐지모드·TLS 가시성 결핍**만으로도 설명되지만,  
> **AI가 우회 조합을 신속·대량으로 탐색·최적화했을 개연성 역시 높습니다.** 따라서 **직접 증거가 확인되기 전이라도** 위험평가와 원인분석에서는 **‘AI 보조 우회’를 기본 가정**으로 삼아야 합니다. 재발 방지를 위해서는 **패치 거버넌스 정상화**와 더불어 **정규화·본문검사·차단 모드·TLS 브리징**을 전제로 한 **계약(화이트리스트) 중심 설계** 및 **응답 DLP/사이즈 관측 상시화**가 필요합니다.
