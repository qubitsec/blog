...
## 8. **PLURA-EDR 관점: “후반”은 충분히 탐지·대응 가능한 사건**
...
* **피싱 저항성 MFA**(FIDO2/보안키 등) 도입 + “전화로 승인 요구 시 무조건 거절” 프로세스화
* **MFA 푸시 승인 정책 강화**(번호 매칭, 승인 사유 표시, 승인 요청 폭주 탐지 등)
* **CRM 내 Export/대량 조회 권한 최소화** + 필요 시 승인 워크플로 적용
* **SaaS 로그(인증/접근/다운로드) → XDR/SIEM 연동**으로 “대량 유출 조기 탐지”
* **EDR로 단말 오염 여부 상시 점검** + 사고 시 증거(로그) 확보 체계화 ([Plura][2])
