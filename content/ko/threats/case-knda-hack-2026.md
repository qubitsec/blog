...
lastmod: 2026-07-22
draft: false
description: "2025년 4~5월부터 2026년 2월까지 외교부 산하 국립외교원 온라인교육시스템이 장기간 침해된 사건을 공개자료에 근거해 분석합니다. 서버 소프트웨어 제로데이와 보안설정 미비, 정상 권한을 이용한 장기 접근, 전·현직 외교부 및 재외공관 인력의 이름·아이디·이메일·암호화된 비밀번호 유출 정황, 초기 6000명 보도와 최대 1만 건 설명의 차이, 공격자 귀속과 실제 반출 범위의 미확정 사항, 필요한 원본 로그와 PLURA-WAF·EDR·XDR 관점의 대응 과제를 구분해 정리합니다."
featured_image: "/cdn/threats/case-knda-hack-2026.png"
tags: ["국립외교원", "외교부", "재외공관", "온라인교육시스템", "제로데이", "서버 소프트웨어", "보안설정 미비", "개인정보 유출", "장기침투", "외교관", "국방무관", "스피어피싱", "MITRE ATT&CK", "T1190", "T1213", "PLURA-XDR", "PLURA-WAF", "PLURA-EDR"]
---
...
- **핵심 교훈:** 패치가 없는 제로데이라도 웹 요청·서버 프로세스·계정·데이터베이스·외부 통신을 함께 분석하면 침해 이후의 비정상 행위를 탐지할 수 있어야 합니다.
...
현재 공개자료만으로는 공급망 공격이라고 단정할 수 없습니다.
...
- 서명 인증서 탈취
- 오픈소스 패키지 또는 의존성 오염
- 관리서비스 제공업체를 통한 다수 고객 침해

이번 사건에서 확인된 것은 다음입니다.

```text
외교부가 운영한 서버 소프트웨어에 제로데이 취약점이 있었고,
공격자가 이를 국립외교원 시스템에 악용했다.
```
...
- 공급사가 해킹됐는지
- 악성 업데이트가 배포됐는지
- 동일 제품의 다른 고객이 동시에 침해됐는지
- 공급망 단계에서 코드가 변조됐는지

따라서 정확한 표현은 **서버 소프트웨어 제로데이 악용 사건**입니다.

제품 공급사 침해나 악성 업데이트가 추가로 확인될 때 공급망 공격 여부를 다시 판단해야 합니다.
...
22. 시스템은 정기 보안점검 대상이었는가?
23. 점검이 있었다면 어떤 항목과 로그를 확인했는가?
24. 왜 과거 사용자와 전직자 정보가 남아 있었는가?
25. 누가 자산·패치·관제·개인정보 파기를 책임졌는가?
26. 관계기관 통보 전 자체 탐지가 없었던 이유는 무엇인가?

### 공격자 귀속

27. 공격 IP·도메인·악성코드·도구는 무엇인가?
28. 기존 국가 배후 캠페인과 겹치는 인프라가 있는가?
29. 북한·중국 또는 다른 조직을 특정할 수 있는 증거가 있는가?
30. 최종 기술보고서를 어느 범위까지 공개할 것인가?
...
같은 소프트웨어와 설정을 사용하는 다른 정부기관은 안전한가.**
